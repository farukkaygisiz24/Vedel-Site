...
function KlimaSecBanner() {
    return (
        <div className="my-8 bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl p-6 md:p-8 text-white shadow-lg">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-orange-100 mt-1">Klimanızı seçmenizde size yardımcı olalım! İhtiyacınıza en uygun klimayı birlikte bulalım.</p>
                    </div>
                </div>
                <Link
                    href="/klimani-sec"
                    className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-white text-orange-600 font-semibold rounded-xl hover:bg-orange-50 transition-colors"
                >
                    Klimanı Seç
...
function ProductCard({ product }: { product: Product }) {
    ...
    return (
        <Link href={`/urunler/euroform/${modelParam}`}>
...
export default function EuroformSplitClient({ data }: { data: EuroformSplitData }) {
    const [filteredProducts, setFilteredProducts] = useState<Product[]>(data.split || [])
                ...
                <div className="flex-1">
                    ...
                    <section className="mb-12">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900">Wing Serisi</h2>
                                <p className="text-gray-500">Enerji verimli ve ekonomik split klima çözümleri</p>
                            </div>
                            <span className="text-sm text-gray-400">{filteredProducts.length} ürün</span>
                        </div>
