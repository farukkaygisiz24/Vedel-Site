...
function KlimaSecBanner() {
    return (
        <div className="my-8 bg-gradient-to-r from-teal-600 to-teal-700 rounded-2xl p-6 md:p-8 text-white shadow-lg">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-bold">Profesyonel Klimanızı Seçmekte Zorlanıyor musunuz?</h3>
                        <p className="text-teal-100 mt-1">İşletmenize en uygun klimayı seçmenizde size yardımcı olalım!</p>
                    </div>
                </div>
                <Link
                    href="/klimani-sec"
                    className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-white text-teal-700 font-semibold rounded-xl hover:bg-teal-50 transition-colors"
                >
                    Klimanı Seç
            ...
            <div className="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full">
                ...
                <div className="p-6">
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            <span className="inline-block px-3 py-1 bg-teal-100 text-teal-700 text-xs font-bold rounded-full mb-2">
                                {product.btu} BTU
                            </span>
                            ...
                            <section key={key} className="mb-12">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                    {filteredCategoryProducts.map((product) => (
                                        <ProductCard key={product.id} product={product} brand="euroform_profesyonel" />
                                    ))}
                                </div>
                            </section>
                        )
                ...
                <div className="grid md:grid-cols-2 gap-8 items-center">
                    <div>
                        ...
                        <p className="text-gray-600 mb-6">
                            Uzman ekibimiz ihtiyaçlarınızı analiz ederek en uygun profesyonel klima çözümünü sunmaktadır.
                        </p>
                        <Link
                            href="/iletisim"
