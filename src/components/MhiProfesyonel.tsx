...
function KlimaSecBanner() {
  return (
    <div className="my-8 bg-gradient-to-r from-purple-600 to-purple-700 rounded-2xl p-6 md:p-8 text-white shadow-lg">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-purple-100 mt-1">İşletmenize en uygun klimayı seçmenizde size yardımcı olalım!</p>
          </div>
        </div>
        <Link
          href="/klimani-sec"
          className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-white text-purple-700 font-semibold rounded-xl hover:bg-purple-50 transition-colors"
        >
          Klimanı Seç
              ...
              <section key={key} className="mb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filteredCategoryProducts.map((product) => (
                    <ProductCard key={product.id} product={product} brand="mhi_profesyonel" />
                  ))}
                </div>
              </section>
            )
