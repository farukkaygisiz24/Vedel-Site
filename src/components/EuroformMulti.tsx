...
function KlimaSecBanner() {
    return (
        <div className="my-8 bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-2xl p-6 md:p-8 text-white shadow-lg">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-emerald-100 mt-1">Klimanızı seçmenizde size yardımcı olalım! İhtiyacınıza en uygun klimayı birlikte bulalım.</p>
                    </div>
                </div>
                <Link
                    href="/klimani-sec"
                    className="shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-white text-emerald-700 font-semibold rounded-xl hover:bg-emerald-50 transition-colors"
                >
                    Klimanı Seç
...
interface EuroformMultiData {
    ...
    categories: {
        outdoor: Product[]
        indoor_wall: Product[]
        cassette_one_way: Product[]
        cassette_four_way: Product[]
        ducted: Product[]
    }
}
                            ...
                            <section key={key} className="mb-12">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                    {filteredCategoryProducts.map((product) => (
                                        <ProductCard key={product.id} product={product} brand="euroform_multi" />
                                    ))}
                                </div>
                            </section>
                        )
